...
RSpec.describe SoftwareRecord, type: :model do
  ...
  it 'is valid if all required fields are provided' do
    softwarerecord = SoftwareRecord.new(title: 'Scholar UC 2', description: 'UC Digital conservatory preservation library', status: 'In Upgrade', software_type_id: SoftwareType.first.id, vendor_record_id: VendorRecord.first.id, created_by: 'Test User2')
    expect(softwarerecord).to be_valid
  end
