...
RSpec.describe User, type: :model do
  describe '.find_or_create_for_shibboleth!' do
    ...
    it 'returns existing users and preserves role' do
      ...
      found_user = User.find_or_create_for_shibboleth!(normalized(identity_attributes))
    ...
    it 'creates new users as active viewers' do
      created_user = User.find_or_create_for_shibboleth!(normalized(identity_attributes))
    ...
    it 'allows first-time users from non-uc domains' do
      created_user = User.find_or_create_for_shibboleth!(normalized(identity_attributes.merge(eppn: 'manager@example.com', email: 'manager@example.com')))
    ...
    it 'uses provided normalized identity when supplied' do
      ...
      created_user = User.find_or_create_for_shibboleth!(normalized_identity)
    ...
    it 'uses email as eppn fallback when eppn is blank' do
      created_user = User.find_or_create_for_shibboleth!(normalized(identity_attributes.merge(eppn: nil, email: 'jane.doe@example.com', first_name: 'Jane', last_name: 'Doe')))
    ...
    it 'builds fallback eppn/email from names when identity values are blank' do
      created_user = User.find_or_create_for_shibboleth!(normalized(identity_attributes.merge(eppn: nil, email: nil, first_name: 'Jane', last_name: 'Doe')))
    ...
    it 'uses blank-name fallbacks when creating fallback eppn/email' do
      created_user = User.find_or_create_for_shibboleth!(normalized(identity_attributes.merge(eppn: nil, email: nil, first_name: nil, last_name: '')))
    ...
    it 'treats null-like identity values as missing for eppn/email fallback' do
      created_user = User.find_or_create_for_shibboleth!(normalized(identity_attributes.merge(eppn: '(null)', email: '(null)', first_name: 'null', last_name: 'undefined')))
    ...
    it 'finds existing users by eppn even when email differs' do
      ...
      found_user = User.find_or_create_for_shibboleth!(normalized(identity_attributes.merge(eppn: 'manager@uc.edu', email: 'new@example.com')))
    ...
    it 'links legacy email-only users by assigning eppn on first shibboleth login' do
      ...
      found_user = User.find_or_create_for_shibboleth!(normalized(identity_attributes.merge(eppn: 'legacy@uc.edu', email: 'legacy@example.com')))
    ...
    it 'raises a friendly error when email matches but eppn conflicts' do
      ...
      expect do
        User.find_or_create_for_shibboleth!(normalized(identity_attributes.merge(eppn: 'new@uc.edu', email: 'legacy@example.com')))
