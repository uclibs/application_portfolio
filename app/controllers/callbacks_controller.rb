...
def shibboleth
  ...
  request.env.each do |k, v|
    Rails.logger.info "#{k}: #{v}" if k.to_s.start_with?('HTTP_') || k =~ /shib|eppn|mail/i
  end
  ...
  if @user.persisted?
    ...
    session["devise.shibboleth_data"] = auth.except("extra")
    redirect_to new_user_registration_url, alert: "There was a problem signing you in through Shibboleth."
  end
end

def passthru
  request.env.select { |k, _| k.to_s.include?('HTTP_') || k =~ /shib/i }
  Rails.logger.info ">>> HIT PASSTHRU <<<"
   Rails.logger.info ">>> HEADERS <<<"
   request.env.each do |k, v|
     Rails.logger.info "#{k}: #{v}" if k.to_s.start_with?('HTTP_') || k =~ /shib|eppn|mail/i
  end
...
#  render plain: "You're not authenticated via Shibboleth.", status: 404
